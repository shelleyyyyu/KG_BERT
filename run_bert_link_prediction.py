...
def main():
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        tokenizer = BertTokenizer.from_pretrained(args.output_dir, do_lower_case=args.do_lower_case,cache_dir=cache_dir)
    else:
        model = BertForSequenceClassification.from_pretrained(args.bert_model, num_labels=num_labels,cache_dir=cache_dir)
    model.to(device)
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        # Load a trained model and vocabulary that you have fine-tuned
        model = BertForSequenceClassification.from_pretrained(args.output_dir, num_labels=num_labels,cache_dir=cache_dir)
        tokenizer = BertTokenizer.from_pretrained(args.output_dir, do_lower_case=args.do_lower_case,cache_dir=cache_dir)
        model.to(device)
